# Frankston Granny Flats — rank-and-rent microsite
...
| Item | Value |
|---|---|
| Domain | `frankstongrannyflats.com.au` (registered, GoDaddy fraud-pending) |
| Live URL (interim) | https://frankstongrannyflats.netlify.app |
...
├── pages/
│   ├── index.astro           # home (Frankston primary)
│   ├── officer.astro         # /mornington/
│   ├── berwick.astro         # /frankston-south/
│   ├── beaconsfield.astro    # /carrum-downs/
│   ├── cardinia.astro        # /mornington-peninsula/ (shire-wide overview)
...
├── whisper.js                # "Pete's Lead from Frankston Granny Flats" played to contractor before patching caller through
...
| Telnyx TeXML app | `FrankstonRetainingWalls-Voicemail` (id `2955788144311535476`) |
...
| `TENANT_BRAND` | `Frankston Granny Flats` (default) | What plays after the label — full whisper is `"Pete's Lead. Pete's Lead from Frankston Granny Flats."` |
...
3. Submit to Google Search Console (`sc-domain:frankstongrannyflats.com.au`).
4. Set up Google Business Profile (needs registered business name "Frankston Granny Flats" first).
5. Backlink work: hipages directory, True Local AU, Yelp AU, Frankston City community pages.
6. Shortlist 2–3 retaining wall contractors in Frankston; pitch the leads with a $1,500–$2,500/mo exclusive retainer.

## Niche-fit notes

| Factor | Why this niche |
|---|---|
| Local fit | Frankston/Mornington estates are sloped — demand is structural, not weather-cyclical |
